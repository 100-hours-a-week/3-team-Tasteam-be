...
|---|-----------|-----------------|-------------|
| 1 | `@ServiceIntegrationTest` | 없음 | 20 (FakeNaverGeocodingClient·FakeRestaurantEventPublisher·FakeStorageClient 공유) |
| 2 | `@ServiceIntegrationTest` + `@TestPropertySource` | 없음 | 1 (GroupEmailRateLimitIntegrationTest — @TestPropertySource로 분리) |
| 3 | `@RepositoryJpaTest` | 없음 | 16 |
| 4 | `@ControllerWebMvcTest` | 16개 (BaseControllerWebMvcTest 공통) | 13 |
| 5 | 커스텀 `@SpringBootTest` | - | 1 |
| 6 | 커스텀 `@SpringBootTest` | - | 1 |

**이전 대비 개선:** 컨텍스트 9개 → 7개 (-2). `@ServiceIntegrationTest` 컨텍스트 5개 → 2개.

**개선 방법:** `FakeNaverGeocodingClient`, `FakeRestaurantEventPublisher`를 `@Component @Primary @Profile("test")`로 추가.
`@MockitoBean`을 사용하던 5개 테스트 클래스에서 해당 선언 제거.
