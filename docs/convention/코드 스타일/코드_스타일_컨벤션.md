...
- Eclipse 포매터 기반으로 동작
- `config/formatter/naver-eclipse-formatter.xml` 사용
...
2. 포매팅 파일 지정
   - `config/formatter/intellij-java-wooteco-style.xml` 파일 경로는 IntelliJ에서 참조용으로 남겨두었습니다 (Spotless는 `naver-eclipse-formatter.xml`을 사용합니다).
3. Import 순서 설정
