...
| `domain/search/repository/impl/SearchQueryRepositoryImpl.java` | 수정 | FTS 전략 분기 + `executeFtsNativeStrategy()` + `buildFtsMvRankedSql()` + `cursorScoreFts()` |
| `domain/search/service/SearchDataService.java` | 수정 | `ftsRank` 커서 설정 + `@Cacheable` 첫 페이지 캐시 |
| `domain/search/scheduler/SearchMvRefreshScheduler.java` | 신규 | MV CONCURRENTLY refresh 스케줄러 |
| `resources/application.yml` | 수정 | 기본 전략 `FTS_MV_RANKED`, MV refresh 설정, 캐시 TTL 항목 추가 |
...
**하위 호환성**: 기존 전략(ONE_STEP~MV_SINGLE_PASS)은 `ftsRank=null`로 반환. `executeNativeStrategy()`에서 `null` 고정 삽입. QueryDSL 전략에서는 `Expressions.nullExpression(Double.class)` 사용. 기존 `cursorScore()` 메서드는 변경하지 않고 FTS 전략 전용 `cursorScoreFts()`를 별도 정의.
