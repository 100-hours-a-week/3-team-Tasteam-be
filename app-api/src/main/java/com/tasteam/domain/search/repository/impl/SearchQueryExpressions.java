...
public final class SearchQueryExpressions {
	...
	static final double MIN_NAME_SIMILARITY = 0.3;
	...
	public static NumberExpression<Double> totalScore(
		NumberExpression<Integer> nameExactScore,
		NumberExpression<Double> nameSimilarity,
		NumberExpression<Double> distanceMeters,
		Double radiusMeters) {
	...
	public static BooleanExpression cursorCondition(SearchCursor cursor, QRestaurant r,
		...
		double score = cursorScore(cursor, radiusMeters);
		BooleanExpression cond = null;
		cond = or(cond, totalScore.lt(score));
		cond = or(cond, totalScore.eq(score).and(r.updatedAt.lt(cursor.updatedAt())));
		cond = or(cond, totalScore.eq(score)
	...
	public static List<OrderSpecifier<?>> scoreOrderSpecifiers(QRestaurant r, NumberExpression<Double> totalScore) {
