...
public interface MenuControllerDocs {
	...
	SuccessResponse<RestaurantMenuResponse> getRestaurantMenus(
		...
		Long restaurantId,
		@Parameter(description = "비어있는 카테고리 포함 여부", schema = @Schema(defaultValue = "false"), example = "false") @RequestParam(defaultValue = "false")
		boolean includeEmptyCategories,
		@Parameter(description = "추천 메뉴 우선 정렬 여부", schema = @Schema(defaultValue = "true"), example = "true") @RequestParam(defaultValue = "true")
