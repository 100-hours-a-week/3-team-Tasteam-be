#!/bin/bash
#
# 클라이언트 이벤트 Kafka publish spike 테스트 실행 스크립트
#
...
export TEST_TYPE="${TEST_TYPE:-mixed-identity-spike}"
...
echo ""
echo "🔥 클라이언트 이벤트 Kafka publish spike 테스트 시작..."
echo "   스크립트: ${SCRIPT_DIR}/client_activity_kafka_publish_spike_test.js"
...
TEST_ID="client-activity-kafka-publish-$(date +%Y%m%d-%H%M%S)"
...
k6 run $K6_OUTPUT_ARG \
  ...
  client_activity_kafka_publish_spike_test.js

echo ""
echo "✅ 클라이언트 이벤트 Kafka publish spike 테스트 완료!"
