#!/bin/bash
#
# 클라이언트 이벤트 DB 저장 경로 집중 부하 테스트 실행 스크립트
#
...
export TEST_TYPE="${TEST_TYPE:-store-mixed-realistic}"
...
echo ""
echo "🔥 클라이언트 이벤트 DB 저장 부하 테스트 시작..."
echo "   스크립트: ${SCRIPT_DIR}/client_activity_db_store_stress_test.js"
...
TEST_ID="client-activity-db-store-$(date +%Y%m%d-%H%M%S)"
...
k6 run $K6_OUTPUT_ARG \
  ...
  client_activity_db_store_stress_test.js

echo ""
echo "✅ 클라이언트 이벤트 DB 저장 부하 테스트 완료!"
