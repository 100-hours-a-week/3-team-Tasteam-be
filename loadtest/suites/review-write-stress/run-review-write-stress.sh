...
echo ""
echo "🔥 리뷰 작성 집중 부하 테스트 시작..."
echo "   스크립트: ${SCRIPT_DIR}/review_write_stress_test.js"
...
TEST_ID="review-write-stress-$(date +%Y%m%d-%H%M%S)"

cd "$SCRIPT_DIR"
k6 run $K6_OUTPUT_ARG \
  --tag testid=$TEST_ID \
  -e TEST_ID=$TEST_ID \
  review_write_stress_test.js

echo ""
echo "✅ 리뷰 작성 집중 부하 테스트 완료!"
